...
from app.schemas.schema import ComentariosSchema, PosteosSchema, PrivateUserSchema, PublicUserSchema, CategoriaShema
...
class MetodoComentario(MethodView):
    def get(self):
        all_comentario= Comentario.query.all()
        comentario_shema = ComentariosSchema().dump(all_comentario, many=True)
        return jsonify(comentario_shema), 201
    
    def post(self):
        comentarios=request.get_json()
        id=comentarios.get("id")
        texto=comentarios.get("texto")
        fecha_creacion=comentarios.get("fecha_creacion")
        autor_id=comentarios.get("autor_id")
        autor=comentarios.get("autor")

        nuevo_comentario = Comentario(id=id, texto=texto, fecha_creacion=fecha_creacion, autor_id=autor_id, autor=autor)
         
        db.session.add(nuevo_comentario)
        db.session.commit()

        return jsonify(crear="comentario listo"), 201
...
app.add_url_rule("/add_comentario", view_func=MetodoComentario.as_view("Comentario"))